...
const CategoryCard = ({ title, icon, nested, id, preferences = [], isLoggedIn = false }) => {
        ...
        <ul className="pl-6 pb-3 pt-1 -mt-1">
          {nested
            .filter((child) => {
              ...
              const checkNested = (c) => {
                if (preferences.includes(c._id)) return true;
                if (c.children?.length) return c.children.some((nestedChild) => checkNested(nestedChild));
                return false;
              };
            ...
            .map((children) => (
              <li key={children._id}>
                {children.children.length > 0 ? (
                  <a
                    onClick={() => handleSubNestedCategory(children._id, showingTranslateValue(children.name))}
                  ...
                  <a
                    onClick={() => handleSubCategory(children._id, showingTranslateValue(children.name))}
                ...
                {showSubCategory.id === children._id && showSubCategory.show === true ? (
                  <ul className="pl-6 pb-3">
                    {children.children.map((subChildren) => (
                      <li key={subChildren._id}>
                        <a
                          onClick={() => handleSubCategory(subChildren._id, showingTranslateValue(subChildren?.name))}
                          className="flex items-center font-serif py-1 text-sm text-gray-600 hover:text-emerald-600 cursor-pointer"
                        >
                          <span className="text-xs text-gray-500 pr-2">
                            <IoRemoveSharp />
