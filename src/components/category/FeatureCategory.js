          ...
          ?.map((category) => {
            const filteredChildren =
              category.children?.filter(
                (child) =>
                  preferences.includes(child._id) ||
                  child.children?.some((subChild) => preferences.includes(subChild._id))
              ) || [];

            if (preferences.includes(category._id) || filteredChildren.length > 0) {
                  ...
                  <div className="pl-4">
                    ...
                    <ul className="pt-1 mt-1">
                      {category?.children?.slice(0, 3).map((child) => (
                        <li key={child._id} className="hover:text-orange-500 hover:ml-2 transition-all duration-150">
                          <a
                            onClick={() => handleCategoryClick(child._id, showingTranslateValue(child?.name))}
