    ...
    <>
      {option === 'Dropdown' ? (
        <select
          ...
          name="parent"
        >
          {[...new Map(variants.map((v) => [v[att], v].filter(Boolean))).values()].filter(Boolean).map(
            (vl, i) =>
              ...
              varTitle.map((vr) =>
                vr?.variants?.map(
                  (el) =>
                    vr?._id === att &&
                    el?._id === vl[att] && (
                      <option key={i + 1} value={selectVariant[att]} defaultValue={selectVariant[att]} hidden>
          ...
          {[...new Map(variants.map((v) => [v[att], v].filter(Boolean))).values()].filter(Boolean).map((vl, i) =>
            varTitle.map((vr) =>
              vr?.variants?.map(
                (el) =>
                  vr?._id === att &&
