...
const useCheckoutSubmit = (storeSetting) => {
  ...
  const submitHandler = async (data, selectedDeliveryPoint) => {
    console.log('Selected delivery point:', selectedDeliveryPoint);
    ...
    try {
      ...
      switch (data.paymentMethod) {
        case 'Card':
          ...
          break;
        case 'QR':
          await handlePaymentWithQR(orderInfo);
          break;
        case 'Cash':
  ...
  const handleOrderSuccess = async (orderResponse, orderInfo) => {
    try {
      ...
      if (globalSetting?.email_to_customer) {
        ...
        OrderServices.sendEmailInvoiceToCustomer(updatedData).catch((emailErr) => {
  ...
  const handlePaymentWithQR = async (orderInfo) => {
    try {
      const orderResponse = await OrderServices.addOrder({
        ...orderInfo,
        paymentMethod: 'QR',
      ...
      notifySuccess('Please complete the payment by scanning the QR code.');
      emptyCart();
      setIsCheckoutSubmit(false);
    } catch (err) {
  ...
  const handleDefaultShippingAddress = (value) => {
    ...
    if (value) {
      ...
      const lastName = nameParts?.length > 1 ? nameParts[nameParts?.length - 1] : ''; // Last name is the last element, if it exists
