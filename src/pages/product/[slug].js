          ...
          p.variants.length <= 1
            ...
            : showingTranslateValue(product?.title) +
              ...
              variantTitle
                ?.map(
                  // (att) => selectVariant[att.title.replace(/[^a-zA-Z0-9]/g, '')]
                  (att) => att.variants?.find((v) => v._id === selectVariant[att._id])
                )
  ...
  return (
    <>
      {isLoading ? (
        ...
        <Layout title={showingTranslateValue(product?.title)} description={showingTranslateValue(product.description)}>
              ...
              <div className="w-full rounded-lg p-3 lg:p-12 bg-white">
                <div className="flex flex-col xl:flex-row">
                  <div className="flex-shrink-0 xl:pr-10 lg:block w-full mx-auto md:w-6/12 lg:w-5/12 xl:w-4/12">
                    ...
                    {product.image[0] ? (
                      <Image src={img || product.image[0]} alt="product" width={650} height={650} priority />
                    ) : (
                      <Image
                    ...
                    {product?.image?.length > 1 && (
                      <div className="flex flex-row flex-wrap mt-4 border-t">
                        <ImageCarousel images={product.image} handleChangeImage={handleChangeImage} />
                      </div>
                    )}
                  </div>
                  ...
                  <div className="w-full">
                    <div className="flex flex-col md:flex-row lg:flex-row xl:flex-row">
                      <div className="xl:pr-6 md:pr-6 md:w-2/3 w-full">
                        ...
                        <Price price={price} product={product} currency={currency} originalPrice={originalPrice} />

                        <div className="mb-4">
                          {variantTitle?.map((a, i) => (
                            <span key={i + 1}>
                              <h4 className="text-sm py-1">{showingTranslateValue(a?.name)}:</h4>
                        ...
                        <div>
                          <div className="text-sm leading-6 text-gray-500 md:leading-7">
                            ...
                            {Object?.keys(product?.description)?.includes(lang)
                              ...
                              : product?.description?.en?.length > 230 && (
                                  <span onClick={() => setIsReadMore(!isReadMore)} className="read-or-hide">
                                    {isReadMore ? t('homeScreen.moreInfo') : t('common:showLess')}
                                  </span>
                                )}
                          </div>
                          ...
                          <div className="flex flex-col mt-4">
                            <span className="font-serif font-semibold py-1 text-sm d-block">
                              <span className="text-gray-800">{t('homeScreen.category')}:</span>{' '}
                              <Link href={`/search?category=${category_name}&_id=${product?.category?._id}`}>
                                <button
                                  type="button"
                          ...
                          <div className="mt-8">
                            <p className="text-xs sm:text-sm text-gray-700 font-medium">
                              {t('homeScreen.callUs')}:{' '}
                              <span className="text-emerald-700 font-semibold">+0044235234</span>{' '}
                            </p>
                          </div>

                          {/* social share */}
                          <div className="mt-2">
                            <h3 className="text-base font-semibold mb-1 font-serif">{t('homeScreen.share')}</h3>
                            <p className="font-sans text-sm text-gray-500">{t('homeScreen.shareText')}</p>
